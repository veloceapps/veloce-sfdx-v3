...
describe('veloce:source:pull', () => {
  it('should pull veloce sources from org', async () => {
    ...
    const cmdResult = await exec('sfdx veloce:source:pull -u ui-autotests -m model:CPQ -p /tmp/');
  ...
  it('should pull UI sources from org', async () => {
    const cmdResult = await exec('sfdx veloce:source:pull -u ui-autotests -m config-ui:OCTA -p /tmp/');
    console.log(cmdResult.stdout);
  });
  it('should pull UI+PML sources from org', async () => {
    const cmdResult = await exec('sfdx veloce:source:pull -u ui-autotests -m model:OCTA,config-ui:OCTA -p /tmp/');
    console.log(cmdResult.stdout);
  });
  it('should pull Configuration Settings sources from org', async () => {
    const cmdResult = await exec('sfdx veloce:source:pull -u ui-autotests -m config-settings -p /tmp/');
    console.log(cmdResult.stdout);
  });
});
