...
`
    ...
    var cmdResult = await exec("sfdx veloce:source:pull -u sfdx-test")
    console.log(cmdResult.stdout)
    // var check = await exec(`sfdx force:data:soql:query  -u sfdx-test-1jul -q "select fields(all) from VELOCPQ__ProductModel__c where Name = '${name}' limit 10" --json`)
