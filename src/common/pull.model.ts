...
async function pullPM(sourcepath: string, conn: Connection, dumpAll: boolean, pmsToDump: Set<string>): Promise<void> {
  ...
  productModels.forEach(({
    ...
    const productModelJson = JSON.stringify({
      Id, Name, VELOCPQ__ReferenceId__c, VELOCPQ__Version__c, VELOCPQ__Active__c, VELOCPQ__BundleProduct__c, VELOCPQ__Comment__c
    }, null, '  ');
...
export async function pullModel(params: PullModelParams): Promise<string[]> {
  ...
  console.log(`Dumping ${dumpAll ? 'All Product Models' : 'Models with names: ' + (Array.from(modelsToDump)?.join() ?? '')}`);
  ...
  const contents: [ProductModel, string|undefined][] = await Promise.all(productModels.map(productModel => Promise.all([
