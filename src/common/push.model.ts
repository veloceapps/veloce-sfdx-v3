...
async function uploadModel(sourcepath: string, conn: Connection, pmlName: string): Promise<string> {
  ...
  const meta = JSON.parse(readFileSync(`${sourcepath}/model/${pmlName}/${pmlName}.json`).toString()) as {
    [key: string]: string;
  };
  ...
  if (documentId !== null) {
    ...
    // update meta
    writeFileSync(`${sourcepath}/model/${pmlName}/${pmlName}.json`, JSON.stringify(meta, null, '  '), { flag: 'w+' });
...
async function uploadPM(sourcepath: string, conn: Connection, pmName: string): Promise<void> {
  ...
  if (pmId === null) {
    ...
    delete meta['Id'];
    await conn.sobject<{ [key: string]: string }>('VELOCPQ__ProductModel__c').create(meta, {}, (err, ret) => {
      if (err || !ret.success) {
