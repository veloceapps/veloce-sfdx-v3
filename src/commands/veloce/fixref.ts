...
export default class Fixref extends SfdxCommand {
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (this.flags.dry) {
      ...
      this.ux.log(
        "Can't provide dry run info about which objects should be fixed, because there is no info in apex debug response, but you can still use wihtout --dry to replace empty ref ids",
      );
      process.exit(1);
    }
