...
export class Script implements OnInit, OnDestroy {
  ...
  private getTableData(lineItem?: LineItem, portName?: string): TableData | undefined {
    ...
    const selectionMode = portDomain.minCard || portDomain.maxCard - portDomain.minCard > 1 ? 'multiple' : 'single';
  ...
  private getProducts(lineItem: LineItem, portDomain: PortDomain, portName: string): any[] {
    ...
    const parentComponent = runtimeModel.components.get(lineItem.type);
    const domain = parentComponent?.ports.find(({ name }) => name === portName)?.domain ?? [];

    return domain.map((typeName) => runtimeModel.components.get(typeName)).filter((c) => !!c);
  }
